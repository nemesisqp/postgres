...
start-fast=y
delta=y
repo1-retention-full=${PGBACKREST_RETENTION_FULL:-8}
...
# Define the 'main' stanza. It will inherit pg1-path from the [global] section.
cat >> "${PGBACKREST_CONFIG}" <<__EOT__
...
# Run a full backup
${PGBACKREST_CRON_FULL} su - postgres -c "pgbackrest --stanza=main --type=full backup --log-level-console=info"

# Run an incremental backup
${PGBACKREST_CRON_INCR} su - postgres -c "pgbackrest --stanza=main --type=incr backup --log-level-console=info"
__EOF__
